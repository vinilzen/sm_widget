var jQueryOrig = null; if(typeof(jQuery) != "undefined"){jQueryOrig = jQuery;}/*! jQuery v1.10.2 | (c) 2005, 2013 jQuery Foundation, Inc. | jquery.org/license */
    ...
    Typeahead.prototype = {
        ...
        constructor: Typeahead, select: function () {
            ...
            $.each(this.source,function(){
                itemsFull.push(this.region);
            });
            items = $.grep(itemsFull, function (item) {
...
}(window.jQuery);/*! Copyright (c) 2011 Brandon Aaron (http://brandonaaron.net)
...
//custom helpers
Handlebars.registerHelper('numberFormat', function(number, decimals, dec_point, thousands_sep) {// Format a number with grouped thousands
	//
...
;
if(jQueryOrig){jQuery = jQueryOrig;}var SmLogger = function() {
    this.widget_id = 'wid';
    this.logLevels = {'off':0,'error':1,'warning':2, 'notice':3};
    this.logStreams = {'console':'console','remote':'remote'};
    this.logLevel = 1;
    this.logStream = 'remote';
    this.remoteUrl = 'http://widget.socialmart.ru/logger.php';
};

SmLogger.prototype.initLogger = function(smJq,options) {
    this.smJq = smJq;
    this.migrateJQuery();
    if(typeof(options.logOptions) == 'object') {
        if(typeof(options.logOptions.level) != 'undefined' && this.hasLogLevel(parseInt(options.logOptions.level))) {
            this.logLevel = options.logOptions.level;
        }
        if(typeof(options.logOptions.stream) == 'string' && typeof(this.logStreams[options.logOptions.stream]) != 'undefined') {
            this.logStream = this.logStreams[options.logOptions.stream];
        }
        if(typeof(options.logOptions.remoteUrl) == 'string' && options.logOptions.remoteUrl) {
            this.remoteUrl = options.logOptions.remoteUrl;
        }
    }
    if(typeof(options.wid) == 'string') {
        this.widget_id = options.wid;
    }


    //Перехватываем все аякс запросы.
    this.smJq.ajaxSetup({
        'widget': this
    });
    if(!this.smJq.ajaxOrig) {
        this.smJq.ajaxOrig = this.smJq.ajax;
        this.smJq.ajax = function(){
            return this.ajaxOrig(arguments[0]).always(
                function(data,status,xhr){
                    if(status == 'success') {
                        this.widget.log('notice','Ajax success',{'url':this.url,'data':data});
                    }
                    else {
                        this.widget.log('error','Ajax fail',{'url':this.url});
                    }
                    return this;
                }
            );
    ...
    //Расширяем jQuery транспорт для jsonp
    this.smJq.ajaxTransport( "+script", function(s) {
        ...
        if ( s.crossDomain ) {
            ...
            return {
                ...
                send: function( _, callback ) {
                    ...
                    script.onerror = script.onreadystatechange  = function(ev) {
                        // Handle memory leak in IE
                        script.onload = script.onreadystatechange = null;
                        // Remove the script
                        if ( script.parentNode ) {
...
SmLogger.prototype.log = function(level,message,obj) {
    var write = false;
    if(typeof(this.logLevels[level]) == 'undefined'){ level = 'error'; }
    if(this.logLevels[level] <= this.logLevel) { write = true ;}
    ...
    if(write) {
        ...
        var d = new Date();
        var data = {
            'timestamp' : d.getTime() - d.getTimezoneOffset(),
            'status'             : level.toUpperCase(),
            'widget_id'         : this.widget_id,
            'location'          : window.location.href,
            'data'                : {'message' : message, 'obj' : obj }
        };
        ...
        switch (this.logStream) {
            case 'console' :
                ...
                break;
            case 'remote' :
                data.data.obj = JSON.stringify(data.data.obj);
                if(encodeURIComponent(JSON.stringify(data)).length > 8100) {
                    data.data.obj = JSON.parse(data.data.obj);
                    console.log(data);
                }
                else {
                    this.smJq.ajaxOrig({
...
SmLogger.prototype.migrateJQuery = function() {
    this.smJq.uaMatch = function( ua ) {
        ua = ua.toLowerCase();
    ...
    if ( !this.smJq.browser ) {
        var matched = this.smJq.uaMatch( navigator.userAgent );
        var browser = {};
        ...
        this.smJq.browser = browser;
    }
};

SmLogger.prototype.hasLogLevel = function(level) {
    for(var prop in this.logLevels) {
        if(this.logLevels.hasOwnProperty(prop) && this.logLevels[prop] === level) {
            return true;
        }
    }
    return false;
...
mixpanel.init("44476cc92036a25e5699e8c702992545");
var smwSkeleton =
        '<div class="smw__stuff__in{{#unless useModelsAccordion}} expand{{/unless}}">' +
            '<div class="smw__body">' +
            '<div class="smw__tab">' +
            '<div class="smw__tab__contents">' +
            '{{#if showOffers}}<div class="smw__tab__contents__item"><div class="smw__prices"><div class="smw__prices__list"></div></div></div>{{/if}}' +
            '{{#if showImpressions}}<div class="smw__tab__contents__item"><div class="smw__impression"></div></div>{{/if}}' +
            '{{#if showOpinerFeedbacks}}<div class="smw__tab__contents__item feedback"><div class="smw__feedback"></div></div>{{/if}}' +
            '{{#if showDetails}}<div class="smw__tab__contents__item"><div class="smw__info-wrap-scroll"></div></div>{{/if}}'+
            '</div></div></div></div>',
    smwHeader =
        ' <div class="smw__header"> ' +
            ...
            '</div>',
    smwTabNav =
        '<ul class="smw__tab__nav">' +
        '{{#if showOffers}}<li><a class="smw__tab__nav__prices  active" href="javascript:void(0);">Лучшие цены<span class="smw__tab__nav__counter"></span> </a></li>{{/if}}' +
        '{{#if showImpressions}}<li><a class="smw__tab__nav__reviews" href="javascript:void(0);">Впечатления <span class="smw__tab__nav__counter"></span> </a></li>{{/if}}' +
        '{{#if showOpinerFeedbacks}}<li><a class="smw__tab__nav__feedbacks opiner" href="javascript:void(0);">Отзывы<span class="smw__tab__nav__counter"></span> </a></li>{{/if}}' +
        '{{#if showDetails}}<li><a class="smw__tab__nav__info " href="javascript:void(0);">Характеристики</a></li>{{/if}}' +
        '</ul>'+
    ...
    smwPriceHead = ' ' +
        ...
        '<strong>Магазины в</strong>' +
        '<div class="where-to-buy "><a class="where-to-buy__trigger" href="javascript:void(0);"></a>' +
        '<form action="#" class="search" onsubmit="javascript:return false;">' +
        '<input autocomplete="off" data-provide="typeaheadSmwMod" data-items="4" type="text" placeholder="Найти город" style="margin:0;"/>' +
    ...
    smwPriceList = '<div class="smw__prices__list-scroll">' +
        ...
        '<div class="smw__prices__item__cost smw__prices__list__cell">{{price}}<small>{{currName}}</small></div>' +
        '<div  class="smw__prices__list__cell"><a class="smw__prices__buy" target="_blank" href="{{clickUrl}}">Купить</a></div>' +
        '</div>'+
        '{{else}}'+
        '<div class="smw__prices__item__virtual">'+
        '<a class="smw__prices__buy" target="_blank" href="{{clickUrl}}">Купить</a>'+
        '</div>'+
        '{{/unless}}'+
        '{{/each }}</script></div></div>'
         ,
    ...
    smwFeedbacksHead =
        '{{#if opinions.length}}' +
        '<div class="smw__feedback__nav">' +
            '<a href="javascript:void(0);" class="smw__feedback__nav__bucket">{{totalRate}}</a>' +
            '<ul class="smw__feedback__marks">' +
                '{{#each opinions}}<li data-category-id="{{categoryId}}">' +
                '<i class="smw__feedback__marks__circle {{markClass}}">{{rate}}</i>' +
                '<span class="smw__feedback__marks__label">{{category}}</span>' +
                '</li>{{/each}}' +
            '</ul>' +
            '<a href="{{allUrl}}" class="smw__feedback__nav__all" style="display: none;"><i></i>ЧИТАТЬ ВСЕ ({{totalCount}})</a>' +
        '</div>' +
        '{{else}}' +
            '<div class="empty">Отзывов для этой модели нет</div>' +
        '{{/if}}',
    smwFeedbacksBody =
        '{{#if opinions.length}}' +
            '{{#each opinions}}<div class="smw__feedback__list-wrap category_{{categoryId}}" style="display: none;">' +
            '<span class="smw__feedback__list__prev"></span>' +
            '<span class="smw__feedback__list__next"></span>' +
            '<div class="smw__feedback__list">' +
                '<ul class="smw__feedback__list__in">' +
                    '{{#each opinion}}' +
                    '<li class="smw__feedback__list__item {{markClass}}">' +
                        '<div class="smw__feedback__list__item__head">' +
                            '<img src="{{userPic}}" alt=""/>' +
                            '<a class="smw__feedback__name" href="javascript:void(0);">{{userName}}</a>' +
                            '<span class="smw__feedback__ya"> на Яндекс.Маркете</span>' +
                        '</div>' +
                        '{{#if isBigOpinion}}<p class="smw__feedback__text full_text">{{fullText}}</a></p>{{/if}}' +
                        '<p class="smw__feedback__text">{{text}}{{#if isBigOpinion}} <a href="javascript:void(0);" class="expandOpinion">>></a>{{/if}}</p>' +
                    '</li>' +
                    '{{/each}}' +
                '</ul>' +
            '</div>' +
        '</div>{{/each}}' +
        '<div class="smw__feedback__list-wrap category_all" style="display: none;">' +
            '<span class="smw__feedback__list__prev"></span>' +
            '<span class="smw__feedback__list__next"></span>' +
            '<div class="smw__feedback__list">' +
                '<ul class="smw__feedback__list__in">' +
                    '{{#each opinions}}{{#each opinion}}' +
                    '<li class="smw__feedback__list__item {{markClass}}">' +
                        '<div class="smw__feedback__list__item__head">' +
                            '<img src="{{userPic}}" alt=""/>' +
                            '<a class="smw__feedback__name" href="javascript:void(0);">{{userName}}</a>' +
        ...
        '</div>'+
        '{{/if}}',
    smwFeedbacks = '<script data-id="feedbackTemplate" type="text/x-handlebars-template">' +
        '{{#if opinions.length}}' + smwFeedbacksHead + smwFeedbacksBody + '{{/if}}'
        + '</script>',
...
;
    ...
    SocialMart.prototype = $smwJq.extend({},new SmLogger(),{

        //list of parameters
        defaults: {
            gadgetName: null,
            urlModel: '/widget/get/model',
            urlModels: '/widget/get/models',
            urlRegions: '/widget/get/regions',
            defaultRegions: ['Москва', 'Санкт-Петербург', 'Красноярск', 'Новосибирск', 'Екатеринбург'],
            regionId: 213,
            displayCustomization : {showTitle : 1, showOffers : 1, showDetails: 1, showImpressions: 1, showOpinerFeedbacks: 0, useModelsAccordion: 1},
            displayMap : {showOffers: 'prices', showDetails: 'info', showImpressions: 'reviews', showOpinerFeedbacks: 'feedbacks'},
            disableRegionSelection: false,
            showWithoutOffers: false,
            scriptsListPath: [
            ...
            cssLinkPath: 'css/style.css?v=1396614820',
            themeLinkPath: 'css/themes/theme_%theme_name%.css?v=1389876078',
            sizeLinkPath: 'css/sizes/size_%size_name%.css?v=1382426488',
        ...
        appendLibraries: function () {
            ...
            if (parent.hasClass('preload')) {
                if(parent.hasClass('preload-js')) {
                    $smwJq.each(self.config.scriptsListPath, function (i, link) {
                        var s = $smwJq('<script></script>', {
                            "src": self.config.linksRoot + link,
                            "charset": "UTF-8"
                        });
                ...
                $smwJq('head').append($smwJq('<link/>', {
                    'href': self.config.linksRoot + self.config.cssLinkPath,
                    'rel': 'stylesheet'
                }));

                $smwJq('head').append($smwJq('<link/>', {
                    'href': self.config.linksRoot + self.config.sizeLinkPath.replace('%size_name%', self.config.sizeName),
                    'rel': 'stylesheet'
                }));

                $smwJq('head').append($smwJq('<link/>', {
                    'href': self.config.linksRoot + self.config.themeLinkPath.replace('%theme_name%',self.config.theme),
                    'rel': 'stylesheet'
                }));

                if(typeof(self.config.customCss) == 'string' && self.config.customCss) {
                    $smwJq('head').append($smwJq('<style/>', {
                        'type' : 'text/css',
                        'text' : self.config.customCss
                    }));
                }
            }
        ...
        createWidget: function () {
            ...
            var self = this,
                $self = this.$elem;
                self.config = $smwJq.extend({}, this.defaults, this.options, this.metadata);
            self.config.displayCustomization = $smwJq.extend({},this.defaults.displayCustomization, this.options.displayCustomization);
            for(var key in self.config.displayCustomization) {
                self.config.displayCustomization[key] = Boolean(parseInt(self.config.displayCustomization[key]));
            }


            self.config.urlModel = self.config.serverUrl+self.config.urlModel;
            ...
            self.gadgetName = this.config.gadgetName ? this.config.gadgetName : '';
        ...
        init: function (self, $self) {
            self.initLogger($smwJq,self.config);
            this.ieFix();
            this.$elem.hide();
            //generate template
            this.render();
            ...
            self.fetchHeaderData().done(function (data) {
                self.fillHeader(data[self.gadgetId], self.templateNames.header);
                self.fetchPricesData().done(function (data) {
                    if(data.region) {
                        self.regionId = data.region_id;
                        self.$elem.find('.where-to-buy__trigger').html(data.region);

                        if(!$smwJq.grep(self.$elem.find('.search input[type=text]').data('typeaheadSmwMod').source, function(e){ return e.region_id == data.region_id; }).length) {
                            self.$elem.find('.search input[type=text]').data('typeaheadSmwMod').source.push({region_id:data.region_id, region:data.region});
                        }
                    }
                    if(data.offers) {
                        self.fillPrices(data.offers, self.templateNames.prices);
                        if (data.offers.length == 0 && self.config.showWithoutOffers == false) {
                            self.$elem.hide();
                        } else {
                            if (self.config.clicksInHeader != false){
                                self.$elem.find('div.smw__header div.smw__stuff-img').addClass('smw__clickable');
                                self.$elem.find('div.smw__header .smw__stuff-name').addClass('smw__clickable');
                                self.$elem.find('div.smw__header .fl_l div').addClass('smw__clickable');
                            }
                        }
                        // статическая кнопка "купить"
                        if (typeof data.staticBuyButton != 'undefined' && data.staticBuyButton > 0) {
                            self.$elem.find('div.smw__prices__list__table').addClass('static');
                        }
                    }
                    self.setActiveTab();
                });
            ...
            self.fetchImpressionsData().done(function (data) {
                if(typeof(data) == 'object') self.fillImpressions(data, self.templateNames.impressions);
            });

            self.fetchInfoData().done(function (data) {
                if(typeof(data) == 'object') self.fillInfo(data, self.templateNames.info);
            });

            self.fetchFeedbacksData().done(function(data){
                if(typeof(data) == 'object') self.fillFeedbacks(data, self.templateNames.feedbacks);
            });
        ...
        _doneCallbacks :  {
            ...
            'feedbacks':  function(){},
            'info' :  function(){},
            'all' : function(){this.$elem.show();}
        },

        _done: function(position) {
            this._doneStates[position] = 1;
            this._doneCallbacks[position].apply(this);
            var isAllDone = true;
            for(var key in this._doneStates) {
                isAllDone = isAllDone && Boolean(this._doneStates[key]);
            }
            if(isAllDone) {
                this._doneCallbacks.all.apply(this);
            }
        },

        done: function (callbackFunction, position){
            if(typeof(position) == 'undefined') {
                position = 'all';
            }
            if(typeof(this._doneCallbacks[position]) == 'function' && typeof(callbackFunction) == 'function') {
                this._doneCallbacks[position] = callbackFunction;
            }
            return this;
        },
        getGadgetId: function (name) {
            ...
            return $smwJq.ajax({
                url: this.config.urlModel + '?name=' + name + '&wId=' + this.wid + "&jsonp=?",
                success: function () {
        ...
        fillRegions: function (regions) {
            ...
            this.$elem.find('.search input[type=text]').typeaheadSmwMod({
                ...
                source: regions,
                items: 4,
                updater: this.switchTownHandler
            });
        ...
        fetchImpressionsData: function () {
            var self = this;
            if(self.config.displayCustomization.showImpressions) {
                return $smwJq.ajax({
                    url: self.config.urlModel + "/impressions?region=" + self.regionId + "&model=" + self.gadgetId + '&wId=' + this.wid + "&jsonp=?",
                    dataType: 'jsonp',
        ...
        fetchFeedbacksData: function () {
            var self = this;
            if(self.config.displayCustomization.showOpinerFeedbacks) {
                return $smwJq.ajax({
                    url: self.config.urlModel + "/feedbacks?region=" + self.regionId + "&model=" + self.gadgetId + '&wId=' + this.wid + "&jsonp=?",
                    dataType: 'jsonp',
                    success: function (data) {
                        self.$elem.find('.smw__tab__nav__feedbacks .smw__tab__nav__counter').html(data.totalCount);
                        self._done('feedbacks');
                    }
                });
            }
            else {
        ...
        fetchHeaderData: function () {
            var self = this;
            if(self.config.displayCustomization.showTitle) {
                return $smwJq.ajax({
                    url: self.config.urlModels + '/info?&jsonp=?',
                    data: {
                        'region': self.regionId,
                        'models': self.gadgetId,
                        'wId': self.wid
                    },
                    dataType: 'jsonp',
                    success: function (data) {
                        data[self.gadgetId].prices.min = self.priceReformat(data[self.gadgetId].prices.min);
                        self.$elem.find('.smw__stuff__num').after(data[self.gadgetId].name);
                        self._done('header');
                    }
                });
            }
        ...
        fetchPricesData: function () {
            var self = this;
            if(self.config.displayCustomization.showOffers) {

                return $smwJq.ajax({
                    url: self.config.urlModels + '/prices?jsonp=?',
                    data: {
                        'region':self.regionId,
                        'models':self.gadgetId,
                        'wId':self.wid,
                        'limit':self.config.offersLimit
                    },
                    dataType: 'jsonp',
                    success: function (data) {
                        if(self.gadgetId in data.models){
                            model = data.models[self.gadgetId];
                            if(!model.total){
                                self.log(
                                    'warning',
                                    'Offers not found',
                                    {url:this.url, 'model':model.id}
                                );
                            }

                            var minPrice = 10000000000;
                            var minOffer = false;
                            $smwJq.map(model.offers, function (offer) {
                                offer.price = self.priceReformat(offer.price)
                                    .replace('руб', ' &nbsp;');
                                offer.shopRatingPercent = offer.shopRating*20;
                                if(parseInt(offer.price.replace(' ','')) < minPrice) {
                                    minPrice = parseInt(offer.price.replace(' ',''));
                                    minOffer = offer;
                                }
                            });

                            // add offers to input data
                            data.offers = model.offers;

                            if (minOffer) {
                                self.$elem.find('.smw__stuff-price').html(minOffer.price+' '+minOffer.currName);
                            }

                            self.pricesCount = model.total;
                            self.$elem.find('.smw__tab__nav__prices .smw__tab__nav__counter').html(model.total);
                            self.$elem.find('.search input[type=text]')[0].value = '';
                        }

                        //remove models from input data
                        delete data.models;
        ...
        fetchInfoData: function () {
            var self = this;
            if(self.config.displayCustomization.showDetails) {
                return $smwJq.ajax({
                    url: self.config.urlModel+'/description?region=' + self.regionId + '&model=' + self.gadgetId + '&wId=' + this.wid + '&jsonp=?',
                    dataType: 'jsonp',
                    success: function(){
                        self._done('info');
                    }
        ...
        fillHeader: function (data, headerTemplateId) {
            if(this.config.displayCustomization.showTitle) {
        ...
        fillFeedbacks: function (data, feedbackTemplateId) {
            this.fillFrag(data, feedbackTemplateId, '.' + this.classNames.feedbacks);
            if (data.opinions.length > 0) {
                this.assignCarousel(this.$elem.find(".smw__feedback__list"), ".smw__feedback__list__prev", ".smw__feedback__list__next");
            } else {
                // если отзывов нет, не показываю вкладку
                this.$elem.find('.smw__tab__nav__feedbacks').parent().hide();
                this.$elem.find('.smw__tab__contents__item.feedback').hide();
            }
        },
        ...
        render: function () {
            var self = this;
            var template = Handlebars.compile(smwSkeleton);
            self.$elem.html(template(self.config.displayCustomization));
            //self.$elem.html(smwSkeleton);
            self.renderHeader();
            self.renderBody();
            // рендер футера только для нескольких моделей и если включена группировка groupOffers
            if (!(self.config.displayCustomization.useModelsAccordion == 0 && self.config.limit > 1)) {
                self.renderFooter();
            }
        },
        ...
        renderBody: function () {
            ...
            var $elem = this.$elem;
            var template = Handlebars.compile(smwTabNav);
            $elem.find('div.smw__tab').prepend(template(this.config.displayCustomization));
            $elem.find('div.smw__prices__list').prepend(smwPriceHead);
            $elem.find('div.smw__prices__list').append(smwPriceList);
            $elem.find('div.smw__tab').append(smwRedirect);
            $elem.find('div.smw__impression').prepend(smwImpressions);
            $elem.find('div.smw__feedback').prepend(smwFeedbacks);
            $elem.find('div.smw__info-wrap-scroll').prepend(smwInfo);
        },
        ...
        attachEvents: function () {
            ...
            self.$elem.on('click', '.smw__feedback__text a.expandOpinion', {self: self }, self.expandOpinion);
            self.$elem.on('click', 'span.smw__feedback__marks__label, i.smw__feedback__marks__circle', {self: self}, self.showByCategory);
            self.$elem.on('click', 'a.smw__feedback__nav__bucket', {self: self}, self.showAllFeedbacks);
            self.$elem.on('click','.smw__prices__item',{self: self}, function(e){
                  var link = $smwJq(this).find('a.smw__prices__buy')[0];
                  if(e.target && e.target == link) {
                      ...
                      myEvt.initEvent('click', false, true);
            ...
            if (self.config.clicksInHeader != false){
                ...
                // Клик на картинке
                self.$elem.on('click','.smw__stuff-img',{self: self}, function(e){
                    var link = self.$elem.find('.smw__prices__list__table a.smw__prices__buy')[0];
                    if (link && link.click) {
                        link.click();
                    }
                });

                self.$elem.on('click', '.smw__header .smw__stuff-name', {self: self}, function(e){
                    var link = self.$elem.find('.smw__prices__list__table a.smw__prices__buy')[0];
                    if (link && link.click) {
                        link.click();
                    }
                ...
                self.$elem.on('click', '.smw__header .fl_l div', {self: self}, function(e){
                    var clickedPrice = self.$elem.find('.smw__header .fl_l .smw__stuff-price')[0];
                    if (clickedPrice){
                        var clickedPriceValue = parseInt(clickedPrice.innerHTML.replace(/ /g, ''));
                        var links = self.$elem.find('.smw__prices__list__table a.smw__prices__buy');
                        var prices = self.$elem.find('.smw__prices__list__table .smw__prices__item__cost');
                        if (links && prices) {
                            var posPrice = -1;
                            $smwJq.each(prices, function(i, price) {
                                if (posPrice == -1) {
                                    var priceValue = parseInt(price.innerHTML.replace(/ /g, ''));
                                    if (priceValue == clickedPriceValue){
                                        posPrice = i;
                                    }
                                }
                            });
                            if (links.length > posPrice){
                                var link = links[posPrice];
                                if (link && links.click) {
                                    link.click();
                                }
                            }
        ...
        switchTownHandler: function(item) {
            var ev = $smwJq.Event('click');
            ev.data = {self:this.context};
            this.context.shownTownFilterHandler(ev);
            var currentRegion = $smwJq.grep(this.source, function(e){ return e.region == item; });
            if(currentRegion.length == 1) {
                var self = this.context;
                self.regionId = currentRegion[0].region_id;
                self.fetchPricesData().done(function (data) {
                    self.$elem.find('.smw__prices__list .smw__prices__list-scroll').remove();
                    self.$elem.find('.smw__prices__list').append(smwPriceList);
                    self.fillPrices(data.offers, self.templateNames.prices);
                    self.setActiveTab();
                });
                self.$elem.find('.where-to-buy__trigger').html(item);
            }
        ...
        sortImpressionsHandler: function (e) {
            ...
            plugin.$elem.find('div.smw__impression__list').html(plugin.$elem.find('div.smw__impression__list__item').sort(plugin.sorter));
            e.preventDefault();
        },
        priceReformat: function (str) {
            // округляю число, отбросив копейки
            str = parseFloat(str).toFixed();
        ...
        setActiveTab: function() {
            ...
            if(!this.pricesCount) {
                this.config.displayCustomization.showOffers = false;
            ...
            for (var key in this.config.displayCustomization) {
                if(this.config.displayCustomization[key]) {
                    if(!isSetActive && this.$elem.find('.smw__tab__nav a.smw__tab__nav__'+this.config.displayMap[key]).length) {
                        this.$elem.find('.smw__tab__nav a.smw__tab__nav__'+this.config.displayMap[key]).trigger('click');
                        isSetActive = true;
                    }
                }
                else {
                    this.$elem.find('div.smw__tab li a.smw__tab__nav__'+this.config.displayMap[key]).parent('li').hide();
                }
            }
        },
        assignCarousel: function(element, prev, next){
            if (typeof $smwJq.jCarouselLite === "undefined") {
                return false;
            }
            var self = this;
            $smwJq.each($smwJq(element), function(i,elem){
                var defaultElementWidth = $smwJq(".smw__feedback__list__item:eq(0)",elem).width(),
                    defaultElementOuterWidth = $smwJq(".smw__feedback__list__item:eq(0)",elem).outerWidth(true),
                    _prev = $smwJq(elem).parent().find(prev),
                    _next = $smwJq(elem).parent().find(next);

                $smwJq(elem).jCarouselLite({
                    visible: 2,
                    scroll: 2,
                    circular: false,
                    btnPrev: _prev,
                ...
                if (i == 0) {
                    $smwJq(elem).parent().show();
                }

                $smwJq.each([_next, _prev], function(index, btn) {
                    self.$elem.on('click', btn, function(){
                        var $expandedLi = $smwJq('li.expanded', elem);
                        if ($expandedLi.length == 0) {
                            return;
                        }

                        $expandedLi.removeClass('expanded').animate({width: defaultElementWidth}, 500, function(){
                            $smwJq($expandedLi.parent()).css('width', $expandedLi.parent().width() - defaultElementOuterWidth);
                        })
                            .find('p.smw__feedback__text').show()
                            .end()
                            .find('p.smw__feedback__text.full_text').hide();
                    });
                });
            });

            this.$elem.find('.smw__feedback__marks li:first').addClass('active');
        },
        expandOpinion: function(e){
            var $target = $smwJq(e.currentTarget),
                $li = $target.parents('li'),
                doubleElementWidth = $li.outerWidth() * 2 - parseInt($li.css('padding-left')),
                $context = $target.parents('.feedback');

            $li.animate({width: doubleElementWidth}, 500, function(){
                $li.find('p.smw__feedback__text').hide()
                    .end().addClass('expanded').css('height','auto')
                    .find('p.smw__feedback__text.full_text').show();

                $smwJq('div.smw__tab__contents__item.feedback', $context).css('height','auto');
            });

            $smwJq($li.parent()).css('width', $li.parent().width() + $li.outerWidth(true));
            // если это последний элемент, делаю хитрую манипуляцию с двумя рядом стоящими отзывами
            if ($li.index() == ($li.parents('ul').children().length - 1)) {
                $li.insertBefore($li.prev());
            }
            $smwJq('.smw__feedback__list', '.smw__feedback__list-wrap:visible').trigger('go', $li.index());
        },
        showByCategory: function(e){
            var $target = $smwJq(e.currentTarget),
                categoryId = $target.parent().data('categoryId'),
                $context = $target.parents('.feedback');

            $smwJq('.smw__feedback__marks', $context).find('li').removeClass('active');
            $target.parent().addClass('active');

            $smwJq('.smw__feedback__list-wrap:visible', $context).hide();
            $smwJq('.smw__feedback__list-wrap.category_' + categoryId, $context).trigger('go',-1).show();
        },
        showAllFeedbacks: function(e){
            var $target = $smwJq(e.currentTarget),
                $context = $target.parents('.feedback');

            $smwJq('.smw__feedback__marks', $context).find('li').removeClass('active');
            $smwJq('.smw__feedback__list-wrap:visible', $context).hide();
            $smwJq('.smw__feedback__list-wrap.category_all', $context).trigger('go',-1).show();
        }
    });

    SocialMart.defaults = SocialMart.prototype.defaults;

    var SocialMartPreBuild = function (elem, options) {
        if(typeof(options.name) != "string" || options.name != "smw" ) {
            return false;
        }
        this.id = [];
        this.frag = [];
        this.elem = elem;
        this.options = $smwJq.extend({},{
            wid : $smwJq(this).attr('data-widget-id'),
            title: $smwJq(document).find('title').text()
        },options);
    };

    SocialMartPreBuild.prototype = $smwJq.extend({},new SmLogger(),{
        init: function () {
            var self = this;
            self.initLogger($smwJq,self.options);
            self.getId().done(function (d) {
                // если передана одна модель
                if (['string', 'number'].indexOf(typeof d.model_id) >= 0) {
                    self.id.push(d.model_id)
                }
                // получаем массив моделей
                else if (typeof d.model_id === 'object') {
                    self.id = d.model_id;
                }
                if(!self.id.length) {
                    self.log('warning','Model not found',{url:this.url});
                }
                $smwJq.each(self.id, function (i, el) {
                    if(self.id.hasOwnProperty(i)) {
                        var frag = self.createFrag(el);
                        self.frag.push(frag);
                        $smwJq(self.elem).append(frag);
                        var smw = new SocialMart(frag, self.options);
                        smw.createWidget().done(function(){
                            $smwJq(self.elem).show(); //Widget container
                            this.$elem.show(); //Block of each product
                        });
                    }
                })
            });
        ...
        getId: function () {
            var self = this;
            return  $smwJq.ajax({
                url: self.options.serverUrl+'/widget/get/model/?wId='+self.options.wid+'&name=' + encodeURIComponent(self.options.title)+"&limit="+self.options.limit+"&jsonp=?",
                dataType: 'jsonp'
            });
        },
...
(function ($smwJq, window, document, undefined) {
    $smwJq.fn.SocialMart = function (options) {
        ...
        return this.each(function (el, i) {
            var SmPreBuild = new SocialMartPreBuild(this,options);
            SmPreBuild.init();
        });
    };
...
/*
 Test queries
 http://dev2.socialmart.ru/widget/get/model/?name=searchStr
